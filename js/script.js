...
timerEl = document.querySelector('.time span b')

mistakesEl = document.querySelector('.mistake span')
wpmEl = document.querySelector('.WPM span')
cpmEl = document.querySelector('.CPM span')
...
function randomParagraph() {
  // Generate random number that is always less than the length of the paragraph array
  let randIndex = Math.floor(Math.random() * paragraphs.length)
  typingText.innerHTML = ''
  //Get a random paragraph from the paragraphs array, split it into letters and put the letters into span element, then add the span inside of the p tag in typingText.
  //Option 1
  paragraphs[randIndex].split('').forEach((span) => {
    let spanEl = document.createElement('span')
    spanEl.innerHTML = span
    typingText.appendChild(spanEl)
    // Option 2: Using the forEach method to add each character to the typingText element
    // let spanEl = `<span>${span}</span>`
    // typingText.innerHTML += spanEl
  })
  typingText.querySelectorAll('span')[0].classList.add('active')
  // input focus on keydown or click event
  document.addEventListener('keydown', () => inputEl.focus())
  typingText.addEventListener('click', () => inputEl.focus())
}

function initTyping() {
  //Select all character spans when inputEl is focused
  const characters = typingText.querySelectorAll('span')
  //Saved typed character into a variable and add an index
  let typedChar = inputEl.value.split('')[charIndex]
  // Don't let user type when time is out.
  if (charIndex < characters.length - 1 && timeLeft > 0) {
    if (!isTyping) {
      // Once timer started, don't restar again on every key clicked
      timer = setInterval(initTimer, 1000)
      isTyping = true
    }
    // Control flow to decide on correct & incorrect typing.
    // User hasn't entered any character or pressed backspace
    if (typedChar == null) {
      //Decrement the index of a character when backspace is pressed
      charIndex--
      // Decrement the mistakes when backspace is pressed if there is an incorrect character
      ...
      // Control flow for correct / incorrect character matches
      if (characters[charIndex].innerText === typedChar) {
        // Add correct class if the character matched typed character
        characters[charIndex].classList.add('correct')
      } else {
        // Increment mistakes in the lower dashboard if user made a mistake
        mistakes++
        characters[charIndex].classList.add('incorrect')
      }
      charIndex++ // increment charIndex whenever the user types
    ...
    // if wpm value is 0, empty, or infinity then sett its value to 0
    wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm
    //Insert the mistakes number into mistakes span
    mistakesEl.innerText = mistakes
    cpmEl.innerText = charIndex - mistakes // cpm not counting mistakes
    wpmEl.innerText = wpm // cpm not counting mistakes
  } else {
    inputEl.value = ''
    clearInterval(timer)
  }
...
function initTimer() {
  // If timeLeft is great than 0 then decrement the timeLeft else clear the timer
  if (timeLeft > 0) {
    timeLeft--
...
function resetGame() {
  // call loadParagraph function
  // reset each variable and elements value to default to start the new game
  randomParagraph()
  inputEl.value = ''
  clearInterval(timer)
  timeLeft = maxTime
